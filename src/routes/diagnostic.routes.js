...
import { ensureCrmSchema } from './crm.routes.js';
...
const upload = multer({
  storage,
  limits: { fileSize: Number(process.env.MAX_UPLOAD_MB || 120) * 1024 * 1024 },
  ...
  try {
    ...
    try {
      const leadInsert = await pool.query(
        `INSERT INTO leads (full_name, email, company, role_title, whatsapp, consent, crm_stage)
         VALUES ($1, $2, $3, $4, $5, $6, 'lead_entrou')
         RETURNING id`,
      ...
      await pool.query(`INSERT INTO crm_timeline (lead_id, stage, note) VALUES ($1, $2, $3)`, [
        leadInsert.rows[0].id,
        'lead_entrou',
        'Lead capturado via diagnóstico FRAME.',
      ]);

      await pool.query(`INSERT INTO crm_timeline (lead_id, stage, note) VALUES ($1, $2, $3)`, [
        leadInsert.rows[0].id,
        'diagnostico_enviado',
        'Diagnóstico inicial gerado automaticamente pelo fluxo web.',
      ]);

      await pool.query(`UPDATE leads SET crm_stage = 'diagnostico_enviado', crm_last_contact_at = NOW() WHERE id = $1`, [leadInsert.rows[0].id]);
    } catch (dbError) {
